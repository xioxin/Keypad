...
#define SHIFT_DOUBLRE_CLICK_MAX_ROUND   220
...
int doubleClickStatus = 0;
int keypadMode = 0;
int doubleClickRound = 0;
int mouseButtonLock = 0;
...
void loop()
{
  ...
  // 开始双击检查
  if(doubleClickStatus == 0 && shift_key_pressed()) {
    doubleClickRound = 0;
    doubleClickStatus = 1;
    Serial.print("1");
  }

  if(doubleClickStatus == 1 && !shift_key_pressed()) {
    doubleClickStatus = 2;
    Serial.print("2");
  }

  if(doubleClickStatus == 2 && shift_key_pressed()) {
    doubleClickRound = 0;
    doubleClickStatus = 4;
    keypadMode = keypadMode == 1 ? 0 : 1;
    Serial.println(keypadMode == 1 ? 'B' : 'A');
  }
  if (doubleClickStatus > 0 && doubleClickStatus < 4) {
    doubleClickRound++;
    Serial.print('.');
  }
  
  if (doubleClickRound > SHIFT_DOUBLRE_CLICK_MAX_ROUND) {
    doubleClickRound = 0;
    doubleClickStatus = doubleClickStatus == 1 ? 4 : 0;
    Serial.println('!');
  }

  if(doubleClickStatus == 4 && !shift_key_pressed()) {
    doubleClickStatus = 0;
  }
  ...
  if(keypadMode == 0){
    ...
    int upState = digitalRead(3);
    int downState = digitalRead(7);
    int rightState = digitalRead(8);
    int leftState = digitalRead(6);

    int slowClickState = !digitalRead(10);
    int leftClickState = !digitalRead(11);
    int rightClickState = !digitalRead(12);

    int xDistance = (leftState - rightState) * (slowClickState ? 1 : 4);
    int yDistance = (upState - downState) * (slowClickState ? 1 : 4);
...
//      UsbKeyboard.move(xDistance, yDistance);
//      UsbKeyboard.sendMouseEvent(xDistance,yDistance, buttons);
//    }
    UsbKeyboard.sendMouseEvent(xDistance,yDistance, buttons);

//    if(buttons != mouseButtonLock) {
//      if(buttons) {
//        // todo: BUG
//        UsbKeyboard.sendMouseEvent(-1, -1, buttons);
//        UsbKeyboard.update(0);
//        delay(50);
//      }
//      UsbKeyboard.sendMouseEvent(1,1, buttons);
//      mouseButtonLock = buttons;
//    }
  }

  UsbKeyboard.update(220);
  delay(200);
