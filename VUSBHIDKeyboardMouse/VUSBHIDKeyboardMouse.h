 ...
 public:
  ...
  size_t press(uint8_t k) 
  {
    ...
    for(j = 0; j < sizeof(current_keyboard_report.keycode); j++) {
      if(current_keyboard_report.keycode[j] == k) {
        break;
      }
    }
    if (j == sizeof(current_keyboard_report.keycode)) {
      for (i = 0; i < sizeof(current_keyboard_report.keycode); i++) {
        if (current_keyboard_report.keycode[i] == 0x00) {
          current_keyboard_report.keycode[i] = k;
          break;
        }
      }
      if (i == sizeof(current_keyboard_report.keycode)) {
        return 0;
      }
    }
  ...
  size_t release(uint8_t k) 
  {
    ...
    for (i = 0; i < sizeof(current_keyboard_report.keycode); i++) {
      if (0 != k && current_keyboard_report.keycode[i] == k) {
        current_keyboard_report.keycode[i] = 0x00;
      }
    }
    sendKeyReport(&current_keyboard_report);
    //usbSetInterrupt((uint8_t*)&_keyReport, sizeof(_keyReport));
